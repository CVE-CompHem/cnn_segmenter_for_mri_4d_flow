...
def run_inference():
    ...
    logging.info('args.training_output' + args.training_output)
    if os.path.exists(os.path.join(args.training_output,'models/best_dice.ckpt-4160.index')):
        best_dice_checkpoint_path = os.path.join(args.training_output, 'models/best_dice.ckpt-4160')
    ...
    # flow_mri_obj=FlowMRI(geometry,time,time_heart_cycle_period,intensity,velocity_mean,velocity_cov)
    # #Read the hdf5 file
    # flow_mri = flow_mri_obj.read_hdf5(args.inference-input)
    # Option 2
    flow_mri = FlowMRI.read_hdf5(args.inference_input)
    print('shape of input intensity',flow_mri.intensity.shape)
    print('shape of velocity mean', flow_mri.velocity_mean.shape)
    ...
    print('shape of image before pre-processing', image.shape)
    ...
    print('initial target points',target_points.shape)
    ...
    print('shape of image after pre-processing', image.shape)
    ...
    with tf.Graph().as_default():
        ...
        # ============================
        seg_logits, seg_softmax, seg_prediction = model.predict(images_pl, exp_config.model_handle, exp_config.nlabels)
        ...
        image_seg_logits_mri = np.zeros((flow_mri.intensity.shape[3],*flow_mri.intensity.shape[:3], exp_config.nlabels))

        print('before interpolation',orig_image_shape)
        print('target_points',target_points.shape)
        print('image_seg_logits',image_seg_logits.shape)
        print('image_seg_logits_mri',image_seg_logits_mri.shape)#,image_seg_probs_mri.shape)
        # ============================
        ...
        #         rgi = RegularGridInterpolator(points = (np.linspace(flow_mri.geometry[0][0], flow_mri.geometry[0][-1], orig_image_shape[1]),
        #                                        np.linspace(flow_mri.geometry[1][0], flow_mri.geometry[1][-1], orig_image_shape[2]),
        #                                        np.linspace(flow_mri.geometry[2][0], flow_mri.geometry[2][-1], orig_image_shape[3])),
        #                                        values = image_seg_logits[t, ..., v])
        #
        #         image_seg_logits_mri[..., v] = rgi(target_points)
        #     #this line seems wrong, we just need to interpolate seg_mask back to original dimensions
        #     image_seg_probs_mri[t, ...] = sess.run(seg_probs, feed_dict = {logits_pl: image_seg_logits_mri})[..., 0]
        # image_seg_probs = image_seg_probs_mri
        #np.array(np.meshgrid([0, 1], [2, 3], indexing='ij')).transpose([1, 2, 0])

        # target_points = np.array(np.meshgrid(np.linspace(flow_mri.geometry[0][0], flow_mri.geometry[0][-1], orig_image_shape[1]),
        #                 np.linspace(flow_mri.geometry[1][0], flow_mri.geometry[1][-1], orig_image_shape[2]),
        #                 np.linspace(flow_mri.geometry[2][0], flow_mri.geometry[2][-1], orig_image_shape[3]),
        #                 indexing='ij')).transpose([1, 2, 3, 0])
        #
        # print('final target points', target_points.shape)
        #
        # for t in range(image_seg_logits.shape[0]):
        #     for v in range(image_seg_logits.shape[-1]):
        #         rgi = RegularGridInterpolator(points=tuple(flow_mri.geometry), values=image_seg_logits[t, ..., v])
        #         image_seg_logits_mri[t, ..., v] = rgi(target_points)
        #image = image_preprocessed
        #del image_preprocessed

        op_shape=[orig_image_shape[1],orig_image_shape[2],orig_image_shape[3]]
        for t in range(image_seg_logits.shape[0]):
            for v in range(image_seg_logits.shape[-1]):
                image_seg_logits_mri[t, ..., v] = resize(image=image_seg_logits[t, ..., v], output_shape=op_shape, \
                                                         order=1, preserve_range=True, anti_aliasing=True)

        print('after interpolation')
        print('image_seg_probs', image_seg_logits.shape,image_seg_logits_mri.shape)
        #Assigning foreground probability
        image_seg_logits_mri_fg=image_seg_logits_mri[:, :, :, :, 1]
        ...
        segmented_flow_mri = SegmentedFlowMRI(flow_mri.geometry,
                                              ...
                                              image_seg_logits_mri_fg.transpose([1,2,3,0])[...])

        # ============================
        # write SegmentedFlowMRI to file
        # ============================
        inference_output_file = args.inference_output + args.inference_input[:-7] + '_cnn_segmented.h5'
        logging.info('================ Writing SegmentedFlowMRI to: %s ================' + inference_output_file)
        segmented_flow_mri.write_hdf5(inference_output_file)
        logging.info('============================================================')

logging.info('run inf')
run_inference()
logging.info('inf done')